...
+ (instancetype)defaultConfigurationForQuality:(VVLiveAudioQuality)audioQuality{
    ...
    audioConfig.audioFormatID = kAudioFormatMPEG4AAC;//kAudioFormatMPEG4AAC_HE;
    switch (audioQuality) {
...
//    audioConfig.audioSampleRate = [self.class isNewThaniPhone6] ? VVLiveAudioSampleRate_48000Hz : VVLiveAudioSampleRate_44100Hz;
    audioConfig.audioSampleRate = VVLiveAudioSampleRate_44100Hz; // 这里固定为44.1Hz，暂不支持动态改变
...
-(void)setAudioFormatID:(AudioFormatID)audioFormatID
{
    _audioFormatID = audioFormatID;
    
    if (_audioFormatID == kAudioFormatMPEG4AAC_HE) {
        self.mFramesPerPacket = 2048;
    }
    else if (_audioFormatID == kAudioFormatMPEG4AAC)
    {
        self.mFramesPerPacket = 1024;
    }
    else
    {
        self.mFramesPerPacket = 1024;
...
-(void)setAacObjectType:(int)aacObjectType
{
    _aacObjectType = aacObjectType;
    
    if (_aacObjectType == 1) {
        self.mFramesPerPacket = 2048;
    }
    else if (_aacObjectType == 0)
    {
        self.mFramesPerPacket = 1024;
    }
    else
