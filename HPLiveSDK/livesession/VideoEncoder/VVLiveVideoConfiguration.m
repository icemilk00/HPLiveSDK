...
+ (instancetype)defaultConfigurationForQuality:(VVLiveVideoQuality)videoQuality orientation:(UIInterfaceOrientation)orientation{
    ...
    switch (videoQuality) {
        ...
        case VVLiveVideoQuality_Low2:
        {
            ...
            configuration.videoMinBitRate = 300 * 1024;
        }
            break;
    ...
    configuration.videoEncodeFrameRate = configuration.videoFrameRate;
    configuration.videoMinFrameRate = 12;
    configuration.sessionPreset = [configuration supportSessionPreset:configuration.sessionPreset];
    configuration.gop = 1;
    configuration.videoMaxKeyframeInterval = configuration.gop * configuration.videoEncodeFrameRate;
...
- (void)setVideoMaxFrameRate:(NSUInteger)videoMaxFrameRate{
    if(videoMaxFrameRate < _videoEncodeFrameRate || videoMaxFrameRate < 0) return;
    _videoMaxFrameRate = videoMaxFrameRate;
}

- (void)setVideoMinFrameRate:(NSUInteger)videoMinFrameRate{
    if(videoMinFrameRate > _videoEncodeFrameRate || videoMinFrameRate < 0) return;
    _videoMinFrameRate = videoMinFrameRate;
}

-(void)setVideoEncodeFrameRate:(NSUInteger)videoEncodeFrameRate
{
    _videoEncodeFrameRate = videoEncodeFrameRate;
    self.videoMaxFrameRate = _videoEncodeFrameRate + 10;
}
